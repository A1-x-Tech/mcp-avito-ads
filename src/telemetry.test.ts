...
test("opt-out sends nothing", () => {
  ...
  t.send("server_start");
  t.send("tool_call", { tool: "get_balance" });
...
test("startup_failed carries the reason code and no client info", async () => {
  ...
  assert.equal(ping.body.reason, "invalid_account_id");
...
test("unconfigured_start is a valid event and carries its reason", () => {
  ...
  assert.equal(ping.body.reason, "missing_client_id");
});
...
test("server_start carries clientInfo from the MCP handshake", async () => {
  ...
  assert.equal(start.body.app, "mcp-avito-ads");
});
